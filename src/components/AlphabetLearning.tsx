...
const generateQuiz = (group: ArabicLetter[]): QuizQuestion[] => {
  ...
  questions.push({ type: "write", letter: writeLetter });
...
const AlphabetLearning = ({ onClose }: AlphabetLearningProps) => {
  const { t } = useLanguage();
  ...
  return (
    <div ref={sectionRef} className="mt-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-display text-xl font-bold text-foreground">{t("alphabet.title")}</h3>
          <p className="text-sm text-muted-foreground">{t("alphabet.subtitle")}</p>
        </div>
        {onClose && (
        ...
        <div className="space-y-6 animate-[fade-in-up_0.4s_ease-out]">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {currentGroup.map((letter, i) => (
              <button
                key={letter.letter}
                onClick={() => { setCurrentLetterIdx(i); handlePlaySound(letter); }}
                className={`group relative bg-card border rounded-xl p-4 text-center transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 ${
                  currentLetterIdx === i ? "border-accent shadow-sm ring-2 ring-accent/20" : "border-border hover:border-accent/50"
                }`}
              >
                <span className="absolute inset-0 flex items-center justify-center text-6xl font-arabic text-accent/5 pointer-events-none select-none">{letter.letter}</span>
                <div className="relative z-10">
                  <span className="text-4xl font-arabic text-foreground block mb-2">{letter.letter}</span>
                  <span className="text-xs font-semibold text-accent">{letter.name}</span>
                  <span className="block text-xs text-muted-foreground">/{letter.transliteration}/</span>
            ...
            <div className="bg-secondary/30 border border-border rounded-xl p-6 animate-[fade-in-up_0.3s_ease-out]">
              <div className="flex flex-col sm:flex-row items-center gap-6">
                <div className="flex-shrink-0 w-24 h-24 rounded-xl gradient-gold flex items-center justify-center">
                  <span className="text-5xl font-arabic text-accent-foreground">{currentLetter.letter}</span>
                </div>
                <div className="flex-1 text-center sm:text-left">
                  ...
                  <div className="bg-background rounded-lg p-3 inline-block">
                    <span className="text-xs text-muted-foreground">{t("alphabet.example")}:</span>
                    <p className="text-xl font-arabic text-foreground">{currentLetter.exampleWordAr}</p>
                  ...
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-lg gradient-gold text-accent-foreground font-semibold text-xs hover:opacity-90 transition-opacity"
                  >
      ...
      {phase === "quiz" && quizQuestions[quizIdx] && (
        <div className="bg-card border border-border rounded-xl p-6 animate-[fade-in-up_0.4s_ease-out]">
          ...
          {quizQuestions[quizIdx].type === "mcq" && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-1">{t("alphabet.mcqPrompt")}</h3>
              <p className="text-2xl font-arabic text-accent mb-4 font-bold">{quizQuestions[quizIdx].letter.name}</p>
          ...
          {quizQuestions[quizIdx].type === "match" && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-4">{t("alphabet.matchPrompt")}</h3>
                ...
                <div className="space-y-2">
                  {currentGroup.map((l, i) => (
                    <button
                      ...
                      className={`w-full p-3 rounded-xl border text-xl font-arabic transition-all ${
          ...
          {quizQuestions[quizIdx].type === "write" && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-1">{t("alphabet.writePrompt")}</h3>
              <p className="text-5xl font-arabic text-accent mb-4 text-center">{quizQuestions[quizIdx].letter.letter}</p>
              ...
              {writeResult && (
                <div className="mt-4 flex justify-between items-center">
                  <span className={`text-xs font-semibold flex items-center gap-1 ${writeResult === "correct" ? "text-emerald-600" : "text-destructive"}`}>
                    {writeResult === "correct" ? <><CheckCircle className="w-3.5 h-3.5" /> {t("quiz.correct")}</> : <><XCircle className="w-3.5 h-3.5" /> {t("alphabet.correctAnswer")}: {quizQuestions[quizIdx].letter.name}</>}
                  </span>
                  <button onClick={nextQuizQuestion} className="px-4 py-1.5 rounded-lg gradient-gold text-accent-foreground font-semibold text-xs">
                    {t("quiz.next")}
                  </button>
        ...
        <div className="bg-card border border-border rounded-xl p-6 text-center animate-[fade-in-up_0.4s_ease-out]">
          {allDone ? (
            <>
              ...
              <h3 className="font-display text-2xl font-bold text-foreground mb-1">{t("alphabet.allComplete")}</h3>
